...
class EncryptFrame(customtkinter.CTkFrame):
    def __init__(self, master, app, status_bar, **kwargs):
        ...
        # --- NEW: State for step-by-step processing ---
    ...
    def reset_step_state(self, event=None):
        """Resets the step counter and clears visual highlights."""
        self.current_step_index = 0
        # Remove border from all step frames
    ...
    def process_step(self):
        """Processes the recipe one step at a time."""
        self.clear_recipe_errors()  # Clear any previous errors
        ...
        # If we've reached the end, reset and inform the user
        ...
        # Process all steps from the beginning up to the current one
        ...
        for i in range(self.current_step_index + 1):
            ...
            if not success:
                self.app.show_toast("Processing Failed", f"Step '{operation_name}' failed: {result}",
                                    toast_type="error")
                self.reset_step_state()  # Reset on error
                return
            current_data = result
        ...
        # Highlight the step that was just executed
        self.reset_step_state()  # Clear previous highlights
        current_step_frame = recipe_steps[self.current_step_index]
        current_step_frame.configure(border_width=2, border_color="#3498DB")  # Highlight with blue
        self.status_bar.configure(
            text=f"Executed step {self.current_step_index + 1}: {current_step_frame.winfo_children()[1].cget('text')}",
            text_color="gray70")

        # Move to the next step for the next button press
        self.current_step_index += 1

    def bake_recipe(self):
        self.reset_step_state()  # Always reset before a full bake
        # ... (rest of bake_recipe logic is the same) ...
        current_data = self.input_textbox.get("1.0", "end-1c")
        recipe_steps = [child for child in self.recipe_scrollable_frame.winfo_children() if
                        isinstance(child, customtkinter.CTkFrame)]
        if not current_data:
    ...
    def clear_recipe(self):
        ...
        self.recipe_placeholder = customtkinter.CTkLabel(self.recipe_scrollable_frame,
                                                         ...
                                                         text_color="gray60")
        self.update_recipe_placeholder()
        self.reset_step_state()  # Reset state when recipe is cleared

    def add_recipe_step(self, operation_name):
        self.update_recipe_placeholder()
        ...
        if "AES" in operation_name:
            customtkinter.CTkEntry(step_frame, placeholder_text="Enter Key...", width=120).pack(side="left", fill="x",
                                                                                                expand=True, padx=5,
                                                                                                pady=5)
        ...
        self.reset_step_state()  # Reset state when a step is added
    ...
    def create_io_panel(self):
        ...
        # --- Bind the key release event to reset stepping ---
    ...
    def create_recipe_panel(self):
        ...
        # --- Connect the Step button to its function ---
    ...
    def clear_recipe_errors(self):
        for widget in self.recipe_scrollable_frame.winfo_children():
            if isinstance(widget, customtkinter.CTkFrame):
                widget.configure(border_width=0)
                for child_widget in widget.winfo_children():
                    if isinstance(child_widget, customtkinter.CTkLabel) and "error_label" in str(child_widget):
    ...
    def create_operations_sidebar(self):
        ...
        customtkinter.CTkLabel(scrollable_frame, text="Encoders / Decoders",
                               font=customtkinter.CTkFont(weight="bold")).grid(row=0, column=0, pady=(5, 2), padx=10,
                                                                               sticky="w")
        customtkinter.CTkButton(scrollable_frame, text="To Base64", anchor="w",
                                command=lambda: self.add_recipe_step("To Base64")).grid(row=1, column=0, sticky="ew",
                                                                                        padx=10, pady=2)
        customtkinter.CTkLabel(scrollable_frame, text="Ciphers", font=customtkinter.CTkFont(weight="bold")).grid(row=2,
                                                                                                                 column=0,
                                                                                                                 pady=(
                                                                                                                 15, 2),
                                                                                                                 padx=10,
        ...
        customtkinter.CTkButton(scrollable_frame, text="AES Encrypt", anchor="w",
                                command=lambda: self.add_recipe_step("AES Encrypt")).grid(row=3, column=0, sticky="ew",
                                                                                          padx=10, pady=2)
